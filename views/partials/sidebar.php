...
<aside class="flex flex-col items-center bg-slate-600 h-full w-[240px] shadow-lg pt-4">
    ...
    <ul class="links flex flex-col w-full mt-20 gap-y-3 mb-20 text-black text-lg font-semibold">
       ...
       <a href="/settings" class=" pl-6 py-1  w-full flex items-center gap-4  transition-colors  <?=isPath('/settings') ? 'bg-gray-600 text-white' : 'bg-slate-200 hover:bg-gray-600 hover:text-white' ?>">
       <img src="images/settings.png" alt="settings" class="w-5">    
       Settings</a>
       <a href='/archived' class=' pl-6 py-1 bg-slate-200  w-full flex items-center gap-4 hover:bg-gray-600 hover:text-white transition-colors <?=$_SESSION['role']==='a' ?'':'hidden' ?>  <?=isPath('/archived') ? 'bg-gray-600 text-white' : 'bg-slate-200 hover:bg-gray-600 hover:text-white' ?>'>
